...
const registerUser = asyncHandler(async (req, res) => {
  ...
  console.log("email:", email); // Debug log to check the email being processed
  ...
  if (is_User_exist) throw new ApiError(409, "User already exists!"); // Throw an error if the user already exists
  ...
  const coverImage = coverImageLocalFilePath ? await uploadOnCloudinary(coverImageLocalFilePath) : { url: "" };
  ...
  if (!created_user) throw new ApiError(500, "Something went wrong in controller!"); // Throw an error if the user retrieval fails

  // Send a successful response with the created user details
  return res.status(201).json(new ApiResponse(200, created_user, "User registered")); // Send a 201 Created status with user details
});
